...
for n in range (len(nivel1Train)):
	nivel = nivel1Train[n] + nivel2Train[n] + nivel3Train[n] + nivel4Train[n]
	pathTrain.append(int(nivel))

for n in range (len(nivel1Test)):
	nivel = nivel1Test[n] + nivel2Test[n] + nivel3Test[n] + nivel4Test[n]
	pathTest.append(int(nivel))

for n in range (len(nivel1Int)):
	nivel = nivel1Int[n] + nivel2Int[n] + nivel3Int[n] + nivel4Int[n]
...
for h in range (1,18):
	...
	model = HiddenMarkovModel.from_samples(NormalDistribution, n_components=h, X=seq, algorithm='baum-welch')
	model.bake()
	...
	plt.title(label="N comp: "+str(h))
	plt.savefig("img/pomeSinInercia_comp"+str(h)+".png")
	plt.close()